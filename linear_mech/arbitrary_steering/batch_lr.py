...
# for the sake of this experiment, we'll only use the above/below questions to COMPARE
...
print(len(coco_json_data_leftrightonly))
...
for iii, (img_id, caption1, caption2) in enumerate(tqdm(coco_json_data_leftrightonly)):

    dict_of_all_res[(img_id, coco_json_data_leftright_tracker[iii])] = dict()

    image_path = os.path.join(main_img_dir, f"{img_id:012d}.jpg")

    unified_query, subj1, obj1 = make_unified_query(
        caption1, caption2, {"left", "right"}
    )
    ...
    # big_embeds_dicts_foralllayers = torch.load(
    #     "/groups/perona/raphi/vlm-bind-save/harry_extraction/llava.pt",
    #     map_location="cpu",
    #     weights_only=False,
    # )
    ...
    for layer in range(12, 15):  # range(12, 15):  # [5, 14, 16, 20, 30]:
    ...
    torch.save(
        dict_of_all_res, args.save_output_path + "/dict_of_all_res.pt"
    )
    torch.save(
        unified_query_dict, args.save_output_path + "/unified_query_dict.pt"
    )
